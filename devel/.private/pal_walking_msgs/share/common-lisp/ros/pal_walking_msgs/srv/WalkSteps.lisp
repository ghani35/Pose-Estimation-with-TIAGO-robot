...
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql '<WalkSteps-request>)))
  ...
  (cl:format cl:nil "# This service requests that the robot walks~%# The parameters specify how many steps to perform and the length of every step~%~%int32     nsteps~%float64   step_length~%float64   step_time~%~%~%"))
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql 'WalkSteps-request)))
  "Returns full string definition for message of type 'WalkSteps-request"
  (cl:format cl:nil "# This service requests that the robot walks~%# The parameters specify how many steps to perform and the length of every step~%~%int32     nsteps~%float64   step_length~%float64   step_time~%~%~%"))
